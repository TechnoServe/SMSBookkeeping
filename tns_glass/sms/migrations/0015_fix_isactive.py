...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        from sms.models import AmafarangaSubmission, TwakinzeSubmission, SitokiSubmission, IbitumbweSubmission
        AmafarangaSubmission.objects.filter(is_active=False).update(is_active=True)
        IbitumbweSubmission.objects.filter(is_active=False).update(is_active=True)
        TwakinzeSubmission.objects.filter(is_active=False).update(is_active=True)
        SitokiSubmission.objects.filter(is_active=False).update(is_active=True)

        AmafarangaSubmission.all.filter(active=False, is_active=True).update(is_active=False)
        IbitumbweSubmission.all.filter(active=False, is_active=True).update(is_active=False)
        TwakinzeSubmission.all.filter(active=False, is_active=True).update(is_active=False)
        SitokiSubmission.all.filter(active=False, is_active=True).update(is_active=False)
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 3, 11, 15, 24, 55, 496909)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 3, 11, 15, 24, 55, 496825)'}),
