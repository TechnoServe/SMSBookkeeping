...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        from sms.models import IbitumbweSubmission, TwakinzeSubmission, SitokiSubmission, AmafarangaSubmission
        from seasons.models import Season

        season = Season.objects.get(name='2013', country__country_code='RW')

        # fix our wetmill assignments
        season.clone_wetmill_assignments_from_last()

        today = datetime.datetime.now()
        start = today.replace(day=2)

        for sms in IbitumbweSubmission.all.filter(season=season, created__gte=start).order_by('-report_day'):
            # skip if it has errors
            if sms.submission.has_errors:
                continue

            # are there any other active submissions on this day?
            if not IbitumbweSubmission.objects.filter(wetmill=sms.wetmill, report_day=sms.report_day):
                print "ibi approved %s on %s" % (sms.wetmill.name, sms.report_day)
                sms.is_active = True
                sms.active = True
                sms.save()

        for sms in TwakinzeSubmission.all.filter(season=season, created__gte=start).order_by('-report_day'):
            if sms.submission.has_errors:
                continue

            # are there any other active submissions on this day?
            if not TwakinzeSubmission.objects.filter(wetmill=sms.wetmill, report_day=sms.report_day):
                print "twa approved %s on %s" % (sms.wetmill.name, sms.report_day)
                sms.is_active = True
                sms.active = True
                sms.save()

        for sms in SitokiSubmission.all.filter(season=season, created__gte=start).order_by('-start_of_week'):
            if sms.submission.has_errors:
                continue

            # are there any other active submissions on this day?
            if not SitokiSubmission.objects.filter(wetmill=sms.wetmill, start_of_week=sms.start_of_week):
                print "sit approved %s on %s" % (sms.wetmill.name, sms.start_of_week)
                sms.is_active = True
                sms.active = True
                sms.save()

        for sms in AmafarangaSubmission.all.filter(season=season, created__gte=start).order_by('-start_of_week'):
            if sms.submission.has_errors:
                continue

            # are there any other active submissions on this day?
            if not AmafarangaSubmission.objects.filter(wetmill=sms.wetmill, start_of_week=sms.start_of_week):
                print "ama approved %s on %s" % (sms.wetmill.name, sms.start_of_week)
                sms.is_active = True
                sms.active = True
                sms.save()
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 3, 6, 16, 24, 27, 43963)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 3, 6, 16, 24, 27, 43879)'}),
