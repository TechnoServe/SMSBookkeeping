...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        from sms.models import IbitumbweSubmission, AmafarangaSubmission, TwakinzeSubmission
        from wetmills.models import Wetmill
        from seasons.models import Season

        # for each season
        for season in Season.objects.all():
            for wetmill in Wetmill.objects.filter(country=season.country):
                # if this wetmill had at least one amafaranga or ibitumbwe report
                if not IbitumbweSubmission.objects.filter(season=season, wetmill=wetmill) and \
                   not AmafarangaSubmission.objects.filter(season=season, wetmill=wetmill) and \
                   not TwakinzeSubmission.objects.filter(season=season, wetmill=wetmill):
                   wetmill.set_accounting_for_season(season, 'NONE')
    ...
    models = {
        ...
        u'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 11, 27, 11, 24, 12, 35826)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 11, 27, 11, 24, 12, 35305)'}),
